...
function parseTemplate(templateObject: any, valuesObject: any) {
    ...
    const enumeratedParsedValues = templateValues
        ...
        .map((formatString) =>
            stringTemplate(formatString, valuesObject).trim()
        )
        .filter((parsed) => !!parsed)
        .map((parsed, i) => ({ parsed, i }));

    // Return a json object containing each line of the address
    return enumeratedParsedValues.reduce(
        (result, item) => ({
            ...result,
...
function populateIsoMap() {
    ...
    const addIsoToMap = (
        code: string,
        name: string,
        addressFormatOrRelatedISO: string | string[][]
    ) => {
    ...
    addIsoToMap('DK', 'Denmark', [
        [
            optionalPart(honorific + ' ' + title),
            firstName,
            optionalPart(middleName),
            lastName,
        ],
        [optionalPart(companyName)],
        [address1],
